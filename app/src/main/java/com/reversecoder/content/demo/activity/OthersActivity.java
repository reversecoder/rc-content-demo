...
public class OthersActivity extends AppCompatActivity {
    ...
    private void onListItemSelect(int position) {
        ...
        if (hasCheckedItems && mActionMode == null)
            ...
            mActionMode = startSupportActionMode(new ToolbarActionModeCallback(OthersActivity.this, storageListViewAdapter, allOther));
    ...
    public void deleteRows() {
        ...
        for(int i=0;i<selected.size();i++){
            fileInfo = (FileInfo)storageListViewAdapter.getItem(selected.keyAt(i));
            freedSpace=freedSpace+fileInfo.getSize();
        }
        ...
        for (int i = (selected.size() - 1); i >= 0; i--) {
            if (selected.valueAt(i)) {
                //delete from sdcard
                AppUtil.deleteFile(OthersActivity.this, ((FileInfo) storageListViewAdapter.getItem(selected.keyAt(i))).getUri());
...
//        Toast.makeText(MusicActivity.this, selected.size() + " item deleted.", Toast.LENGTH_SHORT).show();
        Toast.makeText(OthersActivity.this, AppUtil.getReadableFileSize((int)freedSpace)+" deleted.", Toast.LENGTH_SHORT).show();
